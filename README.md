...
- **数据源管理**：账号密码登录 Sub2API / New API，按周期采集分组、倍率、余额和能力快照。
- **市场大盘**：统一展示各平台倍率、最低价、平均值和 30 日历史样本。
- **渠道雷达**：生产 Key 与远端分组组成渠道，分别记录主动探测与目标节点真实业务质量。
- **目标节点**：同步 Sub2API 目标分组和既有账号；既有账号只读，并作为保护基线。
...
渠道管家不会修改目标节点上的既有账号。托管账号优先级最低为 `101`，创建后 `schedulable=false`；只有策略判定生成的动作通过上述闸门后才能恢复调度。
