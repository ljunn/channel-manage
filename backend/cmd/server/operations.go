...
func (a *App) createManagedAccount(w http.ResponseWriter, r *http.Request) error {
	...
	var sourceStatus, sourceBase, encryptedKey, keyModels, targetBase string
	var sourceMultiplier sql.NullFloat64
	var targetWrite, manuallyUntrusted bool
	err := a.db.QueryRowContext(r.Context(), `SELECT s.status,s.base_url,k.key_cipher,k.models,t.base_url,t.write_enabled,s.manually_untrusted,sg.multiplier FROM channels c JOIN sources s ON s.id=c.source_id JOIN source_keys k ON k.id=c.source_key_id JOIN source_groups sg ON sg.id=c.source_group_id JOIN targets t ON t.id=$2 WHERE c.id=$1 AND k.production_authorized=true`, input.ChannelID, input.TargetID).Scan(&sourceStatus, &sourceBase, &encryptedKey, &keyModels, &targetBase, &targetWrite, &manuallyUntrusted, &sourceMultiplier)
	if err == sql.ErrNoRows {
	...
	if sourceStatus != "ACTIVE" {
		return &apiError{409, "SOURCE_DELETING", "该数据源正在删除，不能创建托管账号"}
	}
	...
	if !sourceMultiplier.Valid || sourceMultiplier.Float64 < 0 {
		return &apiError{409, "SOURCE_MULTIPLIER_UNAVAILABLE", "渠道尚未获取有效倍率，请先重新扫描数据源"}
	}
	if !targetWrite {
	...
	defer tx.Rollback()
	retryStatusHash := poolModeRetryStatusCodesHash(a.loadPoolModeRetryStatusCodes(r.Context()))
	_, err = tx.ExecContext(r.Context(), `INSERT INTO managed_accounts(id,target_id,channel_id,remote_id,remote_name,platform,priority,concurrency,rate_multiplier,schedulable,ownership_marker,sync_status,model_mapping_hash,pool_mode_retry_status_codes_hash) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,false,$10,'SYNCED',$11,$12)`, id, input.TargetID, input.ChannelID, remoteID, remoteName, targetPlatform, input.Priority, input.Concurrency, sourceMultiplier.Float64, "channel-manage:"+id, managedAccountConfigHash(targetPlatform, modelMap), retryStatusHash)
	if err != nil {
		return err
	}
	for _, groupID := range input.TargetGroupIDs {
