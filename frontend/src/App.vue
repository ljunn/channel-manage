...
const schedulingRows = computed(() => filtered(data.scheduling).filter(item=>(schedulingGroup.value==='all'||item.targetGroupId===schedulingGroup.value)&&(schedulingState.value==='all'||(schedulingState.value==='running'?item.schedulable:!item.schedulable))))
const schedulingPagedRows = computed(() => schedulingRows.value.slice((page.value-1)*pageSize.value,page.value*pageSize.value))
...
watch([schedulingTab,schedulingGroup,schedulingState], () => { page.value=1 })
...
function schedulingReason(row){
  ...
  const wait=row.estimatedValidationSeconds>0&&row.reasons?.some(reason=>reason.includes('有效样本'))?`；预计还需约 ${Math.ceil(row.estimatedValidationSeconds/60)} 分钟积累样本`:''
    ...
    <section class="workspace">
      ...
      <main>
        ...
        <template v-else-if="route==='/scheduling'">
          <div class="page-head"><div><h1>调度运行</h1><span>{{ data.scheduling.filter(item=>item.schedulable).length }} / {{ data.scheduling.length }} 个账号正在参与</span></div><button class="btn" @click="runAutomation"><Play :size="16"/>立即评估</button></div>
          <div class="scheduling-tabs" role="tablist"><button :class="{active:schedulingTab==='status'}" @click="schedulingTab='status'">当前参与状态</button><button :class="{active:schedulingTab==='history'}" @click="schedulingTab='history'">执行记录</button></div>
          <template v-if="schedulingTab==='status'"><div class="source-controls scheduling-controls"><label><span>目标分组</span><select v-model="schedulingGroup"><option value="all">全部分组</option><option v-for="group in schedulingGroups" :key="group.id" :value="group.id">{{ group.name }}</option></select></label><label><span>参与状态</span><select v-model="schedulingState"><option value="all">全部状态</option><option value="running">正在参与</option><option value="stopped">未参与</option></select></label><span class="source-result-count">{{ schedulingRows.length }} 个账号</span></div><StateBlock v-if="!loading&&!schedulingRows.length" title="没有符合条件的托管账号"/><div v-else class="table-wrap"><table><thead><tr><th>托管账号</th><th>来源</th><th>目标分组</th><th>倍率比较</th><th>7 天验证</th><th>当前状态</th><th>判定原因</th></tr></thead><tbody><tr v-for="row in schedulingPagedRows" :key="row.managedAccountId"><td><strong>{{ row.remoteName }}</strong><small>{{ row.targetName }} · 优先级 {{ row.priority }}</small></td><td><strong>{{ row.sourceName }}</strong><small>{{ row.sourceGroup }}</small></td><td><strong>{{ row.targetGroup }}</strong><small>{{ row.policyName||'未配置策略' }}</small></td><td><strong>{{ ratio(row.sourceMultiplier) }} → {{ ratio(row.targetMultiplier) }}</strong><small>源倍率 / 目标上限</small></td><td><strong>{{ row.samples }} / {{ row.minSamples||'--' }} 个样本</strong><small>成功率 {{ row.successRate==null?'--':`${Number(row.successRate).toFixed(1)}%` }} / {{ row.minSuccessRate||'--' }}%</small></td><td><span :class="['badge',row.schedulable?'success':'neutral']">{{ row.schedulable?'正在参与':'未参与' }}</span><small v-if="row.eligible&&!row.schedulable" class="warning-text">等待恢复</small><small v-if="row.schedulable&&!row.eligible" class="warning-text">等待停止</small></td><td class="schedule-reason" :class="{'danger-text':!row.schedulable}">{{ schedulingReason(row) }}</td></tr></tbody></table></div><PaginationBar v-if="!loading&&schedulingRows.length" v-model:page="page" v-model:page-size="pageSize" :total="schedulingRows.length"/></template>
