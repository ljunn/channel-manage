...
function sourceRecommendationText(value){return {INSUFFICIENT_DATA:'数据不足',RECOMMEND_USE:'推荐使用',OBSERVE:'建议观察',STOP_RECHARGE:'建议停止充值'}[value]||'数据不足'}
function sourceRecommendationTone(value){return value==='RECOMMEND_USE'?'success':value==='STOP_RECHARGE'?'danger':value==='OBSERVE'?'warning':'neutral'}
    ...
    <form class="mapping-workspace" @submit.prevent="deploySourceGroups">
        ...
        <div><Network :size="18"/><span><small>目标节点</small><strong>{{ selectedTarget?.name||'尚未选择' }}</strong></span></div>
	      ...
	      <section :class="['source-quality-panel',{untrusted:sourceDetail.source.manuallyUntrusted,paused:sourceDetail.source.schedulingPaused}]">
	        <div class="source-quality-summary"><span :class="['badge',sourceDetail.source.manuallyUntrusted?'danger':sourceRecommendationTone(sourceDetail.source.systemRecommendation)]">{{ sourceDetail.source.manuallyUntrusted?'人工不可信':sourceRecommendationText(sourceDetail.source.systemRecommendation) }}</span><div><strong>{{ sourceDetail.source.manuallyUntrusted?'人工设置已生效':`系统参考 · ${confidenceText(sourceDetail.source.recommendationConfidence)}` }}</strong><small>{{ sourceDetail.source.manuallyUntrusted?'已退出自动调度，余额提醒和新增托管账号已关闭':'系统结论不会自动改变调度或提醒' }}</small></div></div>
	        <ul v-if="!sourceDetail.source.manuallyUntrusted&&sourceDetail.source.recommendationReasons?.length"><li v-for="reason in sourceDetail.source.recommendationReasons" :key="reason">{{ reason }}</li></ul>
