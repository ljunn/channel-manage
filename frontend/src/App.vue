...
async function saveSettings(){const payload={};for(const key of ['shadow_mode','emergency_freeze'])payload[key]=!!data.settings[key];for(const key of ['probe_interval_seconds','scan_interval_seconds','max_daily_probe_cost_usd','min_healthy_channels','confirmation_failures','metric_window_minutes','min_error_samples','error_rate_threshold','balance_alert_threshold'])payload[key]=Number(data.settings[key]);payload.model_quality_probe_model=String(data.settings.model_quality_probe_model||'gpt-5.6-sol').trim();await action(()=>api('/settings',{method:'PATCH',body:body(payload)}),'系统设置已保存')}
...
function dynamicMultiplierRule(row){
  ...
  return `${rule} · 当前目标倍率 ${ratio(row?.targetMultiplier)}`
}
  ...
  <ModalShell v-if="modal==='policy'" title="新建分组策略" @close="close">
    <form class="modal-form" @submit.prevent="createPolicy">
      ...
      <label class="full"><span>分组禁用模型</span><textarea v-model="form.disabledModelsText" rows="6" placeholder="每行输入一个模型名称&#10;例如：&#10;gpt-4o&#10;gpt-4.1"></textarea><small>每行一个，保存时自动忽略空行和重复项</small></label>
	      ...
	      <label><span>倍率上限</span><input :value="form.dynamicMultiplierEnabled&&form.mode==='PRICE'?'由最低价动态计算并写入 Sub2API':'按目标分组自动获取（缓存 3 分钟）'" disabled/></label><label><span>保底可用渠道数</span><input v-model="form.minAvailableChannels" type="number" min="1" max="100" required/></label>
