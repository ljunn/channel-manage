...
async function loadPage(){
  ...
  try{
    ...
    else if(path==='/managed') [data.managed,data.targets,data.channels]=await Promise.all([api('/managed-accounts'),api('/targets'),api('/channels')])
...
async function prepareManaged(){open('managed');if(data.targets[0]){form.targetID=data.targets[0].id;await loadTargetGroups()}}
async function loadTargetGroups(){targetGroups.value=form.targetID?await api(`/targets/${form.targetID}/groups`):[];form.targetGroupIDs=[]}
async function createManaged(){await submit(()=>api('/managed-accounts',{method:'POST',body:body({targetID:form.targetID,channelID:form.channelID,targetGroupIDs:form.targetGroupIDs||[],name:form.name,priority:Number(form.priority||101),concurrency:Number(form.concurrency||1)})}),'托管账号已在目标节点创建，默认停止调度')}
    ...
    <section class="workspace">
      ...
      <main>
        ...
        <template v-if="route==='/overview'">
          ...
          <section class="panel quick-panel"><header><div><span class="eyebrow">WORKFLOW</span><h2>当前流程</h2></div><Activity :size="22"/></header><ol><li class="done"><span>1</span><div><strong>市场采集</strong><small>数据源按周期同步</small></div></li><li class="done"><span>2</span><div><strong>渠道探测</strong><small>质量与模型能力验证</small></div></li><li><span>3</span><div><strong>策略判定</strong><small>生成可解释动作</small></div></li><li><span>4</span><div><strong>审批执行</strong><small>只写入托管账号</small></div></li></ol></section></div></template>
        </template>
        ...
        <template v-else-if="route==='/settings'">
          ...
          <div class="settings-layout"><section class="settings-section"><header><ShieldCheck :size="20"/><div><h2>安全闸门</h2></div></header><label class="toggle-row"><div><strong>影子模式</strong><small>生成动作但不写入目标节点</small></div><input v-model="data.settings.shadow_mode" type="checkbox"/><span/></label><label class="toggle-row danger-row"><div><strong>紧急冻结</strong><small>立即阻止全部远程写动作</small></div><input v-model="data.settings.emergency_freeze" type="checkbox"/><span/></label><label class="toggle-row"><div><strong>自动批准</strong><small>仅在关闭影子模式时生效</small></div><input v-model="data.settings.auto_approve" type="checkbox"/><span/></label></section>
