...
async function loadPage(){
  ...
  try{
    ...
    else if(path==='/settings') { const [settings,notifications,version]=await Promise.all([api('/settings'),api('/notification-channels'),api('/system/version')]);data.settings=settings;if(!data.settings.model_quality_probe_model)data.settings.model_quality_probe_model='gpt-5.6-sol';data.notifications=notifications;Object.assign(systemVersion,version);updateReady.value=!!version.restartPending }
  }catch(reason){showError(reason)}finally{loading.value=false}
}
...
async function saveSettings(){const payload={};for(const key of ['shadow_mode','emergency_freeze'])payload[key]=!!data.settings[key];for(const key of ['probe_interval_seconds','scan_interval_seconds','max_daily_probe_cost_usd','min_healthy_channels','confirmation_failures','transient_confirmation_failures','metric_window_minutes','min_error_samples','error_rate_threshold','balance_alert_threshold'])payload[key]=Number(data.settings[key]);payload.model_quality_probe_model=String(data.settings.model_quality_probe_model||'gpt-5.6-sol').trim();await action(()=>api('/settings',{method:'PATCH',body:body(payload)}),'系统设置已保存')}
...
function modelCheckText(value,required){return {LEGACY:required?'待检测':'未检测（保留现状）',PENDING:'检测排队中',RUNNING:'检测中',PASSED:'检测通过',FAILED:'检测不通过',UNKNOWN:required?'检测失败':'检测失败（保留现状）'}[value]||'需处理'}
function modelCheckTone(value){return value==='PASSED'?'success':value==='FAILED'?'danger':value==='LEGACY'?'neutral':'warning'}
    ...
    <section class="workspace">
      ...
      <main :class="`route-${route.slice(1)}`">
        ...
        <template v-else-if="route==='/settings'">
          ...
          <section class="settings-section probe-model-settings"><header><Brain :size="20"/><div><h2>模型能力检测</h2><small>全局统一的固定检测模型</small></div></header><div class="form-grid"><label class="full"><span>全局检测模型</span><input v-model="data.settings.model_quality_probe_model" type="text" maxlength="200" placeholder="gpt-5.6-sol" required/><small>所有新渠道和手动检测统一使用这个模型；修改后不会自动重跑已有检测，需要在渠道雷达手动触发。</small></label></div></section>
