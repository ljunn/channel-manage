...
async function openPolicy(){open('policy',{targetID:writableTargets.value[0]?.id||'',targetGroupID:'',probeModel:'',disabledModels:[],mode:'PRICE',allowEqualMultiplier:false,minSuccessRate:95,minSamples:5});await loadPolicyTargetGroups()}
async function loadPolicyTargetGroups(){policyTargetGroups.value=form.targetID?await api(`/targets/${form.targetID}/groups`):[];form.targetGroupID='';form.probeModel='';form.disabledModels=[]}
function selectPolicyTargetGroup(){form.probeModel=selectedPolicyTargetGroup.value?.probeModel||selectedPolicyTargetGroup.value?.models?.[0]||'';form.disabledModels=[]}
...
function editPolicy(row){open('policy-edit',{id:row.id,name:row.name,targetName:row.targetName,targetGroupName:row.targetGroupName,probeModels:row.probeModels||[],probeModel:row.config.probeModel||'',disabledModels:[...(row.config.disabledModels||[])],mode:row.config.mode||'PRICE',allowEqualMultiplier:row.config.allowEqualMultiplier===true,minSuccessRate:row.config.minSuccessRate??95,minSamples:row.config.minSamples??5})}
function policyConfigPayload(){return {mode:form.mode,probeModel:form.probeModel,disabledModels:[...(form.disabledModels||[])],allowEqualMultiplier:form.allowEqualMultiplier===true,minSuccessRate:Number(form.minSuccessRate??95),minSamples:Number(form.minSamples??5)}}
...
<template>
  ...
  <ModalShell v-if="modal==='policy'" title="新建分组策略" @close="close">
    <form class="modal-form" @submit.prevent="createPolicy">
      ...
      <label class="full"><span>测试模型</span><select v-model="form.probeModel" required :disabled="!selectedPolicyTargetGroup"><option value="" disabled>请先选择目标分组</option><option v-for="model in selectedPolicyTargetGroup?.models||[]" :key="model" :value="model" :disabled="form.disabledModels?.includes(model)">{{ model }}</option></select></label>
      <fieldset class="policy-model-deny full"><legend>分组禁用模型</legend><div v-if="selectedPolicyTargetGroup?.models?.length" class="policy-model-options"><label v-for="model in selectedPolicyTargetGroup.models" :key="model"><input v-model="form.disabledModels" type="checkbox" :value="model" :disabled="model===form.probeModel"/><span>{{ model }}</span></label></div><small v-else>当前分组没有可配置的模型</small></fieldset>
  ...
  <ModalShell v-if="modal==='policy-edit'" title="编辑分组策略" @close="close">
    <form class="modal-form" @submit.prevent="updatePolicy">
      ...
      <label class="full"><span>测试模型</span><select v-model="form.probeModel" required><option v-for="model in form.probeModels||[]" :key="model" :value="model" :disabled="form.disabledModels?.includes(model)">{{ model }}</option></select></label>
      <fieldset class="policy-model-deny full"><legend>分组禁用模型</legend><div class="policy-model-options"><label v-for="model in form.probeModels||[]" :key="model"><input v-model="form.disabledModels" type="checkbox" :value="model" :disabled="model===form.probeModel"/><span>{{ model }}</span></label></div></fieldset>
      <fieldset class="auth-mode full"><legend>排序模式</legend><label :class="{active:form.mode==='PRICE'}"><input v-model="form.mode" type="radio" value="PRICE"/><span>价格优先</span></label><label :class="{active:form.mode==='SPEED'}"><input v-model="form.mode" type="radio" value="SPEED"/><span>速度优先</span></label></fieldset>
      <label><span>倍率上限</span><input value="按目标分组自动获取（缓存 3 分钟）" disabled/></label><label><span>自动优先级</span><input value="1000 起，每档相差 100" disabled/></label>
      <label class="check-row full"><input v-model="form.allowEqualMultiplier" type="checkbox"/><span><strong>允许等倍率账号参与</strong><small>关闭时源倍率必须严格低于目标分组倍率，避免零差价运行</small></span></label>
      <label><span>最低成功率（%）</span><input v-model="form.minSuccessRate" type="number" min="1" max="100" required/></label><label><span>最少样本</span><input v-model="form.minSamples" type="number" min="1" required/></label>
