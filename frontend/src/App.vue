...
async function loadPage(){
  ...
  try{
    ...
    else if(path==='/settings') { const [settings,notifications,version]=await Promise.all([api('/settings'),api('/notification-channels'),api('/system/version')]);data.settings=settings;if(!data.settings.model_quality_probe_model)data.settings.model_quality_probe_model='gpt-5.6-sol';if(data.settings.pool_mode_retry_status_codes===undefined)data.settings.pool_mode_retry_status_codes='';data.notifications=notifications;Object.assign(systemVersion,version);updateReady.value=!!version.restartPending }
  }catch(reason){showError(reason)}finally{loading.value=false}
}
...
async function saveSettings(){const payload={};for(const key of ['shadow_mode','emergency_freeze'])payload[key]=!!data.settings[key];for(const key of ['probe_interval_seconds','scan_interval_seconds','max_daily_probe_cost_usd','min_healthy_channels','confirmation_failures','transient_confirmation_failures','metric_window_minutes','min_error_samples','error_rate_threshold','balance_alert_threshold'])payload[key]=Number(data.settings[key]);payload.model_quality_probe_model=String(data.settings.model_quality_probe_model||'gpt-5.6-sol').trim();payload.pool_mode_retry_status_codes=String(data.settings.pool_mode_retry_status_codes||'').trim();await action(()=>api('/settings',{method:'PATCH',body:body(payload)}),'系统设置已保存')}
