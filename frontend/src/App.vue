...
async function openPolicy(){open('policy',{targetID:writableTargets.value[0]?.id||'',targetGroupID:'',probeModel:'',disabledModelsText:'',mode:'PRICE',allowEqualMultiplier:false,minSuccessRate:95,minSamples:5});await loadPolicyTargetGroups()}
...
function editPolicy(row){open('policy-edit',{id:row.id,name:row.name,targetName:row.targetName,targetGroupName:row.targetGroupName,probeModel:row.config.probeModel||'gpt-4.1-mini',disabledModelsText:(row.config.disabledModels||[]).join('\n'),mode:row.config.mode||'PRICE',allowEqualMultiplier:row.config.allowEqualMultiplier===true,minSuccessRate:row.config.minSuccessRate??95,minSamples:row.config.minSamples??5})}
function modelNamesFromLines(value){return [...new Set(String(value||'').split(/\r?\n/).map(model=>model.trim()).filter(Boolean))]}
function policyConfigPayload(){return {mode:form.mode,probeModel:form.probeModel.trim(),disabledModels:modelNamesFromLines(form.disabledModelsText),allowEqualMultiplier:form.allowEqualMultiplier===true,minSuccessRate:Number(form.minSuccessRate??95),minSamples:Number(form.minSamples??5)}}
...
<template>
  ...
  <ModalShell v-if="modal==='policy'" title="新建分组策略" @close="close">
    <form class="modal-form" @submit.prevent="createPolicy">
      ...
      <label><span>倍率上限</span><input value="按目标分组自动获取（缓存 3 分钟）" disabled/></label><label><span>自动优先级</span><input value="1000 起，每档相差 100" disabled/></label>
      ...
      <div class="message warning full"><Workflow :size="16"/>启用后自动同步账号启停状态和分档优先级；待恢复渠道最快每 15 秒复检。</div>
