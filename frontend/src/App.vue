  ...
  <ModalShell v-if="modal==='source-detail'&&sourceDetail" :title="sourceDetail.source.name" wide @close="close">
    <form class="mapping-workspace" @submit.prevent="deploySourceGroups">
        ...
        <div><Network :size="18"/><span><small>目标节点</small><strong>{{ selectedTarget?.name||'尚未选择' }}</strong></span></div>
	      ...
	      <section :class="['source-quality-panel',{untrusted:sourceDetail.source.manuallyUntrusted}]">
	        ...
	        <label class="source-trust-toggle"><span><strong>标记为不可信</strong><small>人工设置优先，取消后需重新验证</small></span><input type="checkbox" :checked="sourceDetail.source.manuallyUntrusted" :disabled="sourceTrustBusy" @change="setSourceUntrusted($event.target.checked,$event)"/><i/></label>
	      </section>
