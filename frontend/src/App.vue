...
const eventCategoryLabel = value => ({SOURCE_BALANCE:'账户余额',SOURCE_SCAN:'数据源扫描',TARGET_SYNC:'目标节点同步',GROUP_AVAILABILITY:'分组可用性',ACTION_EXECUTION:'远程调度写入',ACCOUNT_PLATFORM_SYNC:'账号平台校正',ACCOUNT_MODEL_SYNC:'账号模型校正',ACCOUNT_RATE_SYNC:'账号倍率校正',CHANNEL_PROBE:'渠道探测'})[value]||value
...
function defaultPolicyProbeModel(group){return group?.probeModel||group?.models?.[0]||({openai:'gpt-4.1-mini',anthropic:'claude-3-5-haiku-latest',gemini:'gemini-2.5-flash',grok:'grok-3-mini'}[group?.platform]||'gpt-4.1-mini')}
...
async function saveSettings(){const payload={};for(const key of ['shadow_mode','emergency_freeze'])payload[key]=!!data.settings[key];for(const key of ['probe_interval_seconds','scan_interval_seconds','max_daily_probe_cost_usd','min_healthy_channels','confirmation_failures','metric_window_minutes','min_error_samples','error_rate_threshold','balance_alert_threshold'])payload[key]=Number(data.settings[key]);await action(()=>api('/settings',{method:'PATCH',body:body(payload)}),'系统设置已保存')}
...
<template>
    ...
    <section class="workspace">
      ...
      <main :class="`route-${route.slice(1)}`">
        ...
        <template v-else-if="route==='/channels'">
          ...
          <div class="table-wrap"><table class="has-actions"><thead><tr><th>数据源 / Key</th><th>分组</th><th>倍率</th><th>主动探测</th><th>真实业务</th><th>速度基准</th><th>状态</th><th>原因</th><th><span class="sr-only">操作</span></th></tr></thead><tbody><tr v-for="row in paged(data.channels)" :key="row.id"><td><strong>{{ row.sourceName }}</strong><small>{{ row.keyName }}</small></td><td>{{ row.groupName }}</td><td class="ratio">{{ ratio(row.multiplier) }}</td><td>{{ row.successRate==null?'--':`${Number(row.successRate).toFixed(1)}%` }}<small>{{ row.probeSamples1h }} 个样本</small></td><td>{{ row.businessSuccessRate1h==null?'--':`${Number(row.businessSuccessRate1h).toFixed(1)}%` }}<small>{{ row.businessRequests1h }} 个请求</small></td><td>{{ row.speedFirstTokenMs==null?'--':`${(row.speedFirstTokenMs/1000).toFixed(2)} 秒` }}<small v-if="row.speedFirstTokenMs!=null">{{ row.speedMetricSource==='BUSINESS'?'最近业务':'主动探测' }}</small></td><td><span :class="['badge',statusTone(row.lifecycleState)]">{{ statusText(row.lifecycleState) }}</span></td><td>{{ row.stateReason||'--' }}</td><td><div class="row-actions"><button class="icon-btn" title="探测" @click="channelAct(row,'probe')"><Play :size="16"/></button><button v-if="row.lifecycleState!=='HEALTHY'&&row.lifecycleState!=='MANUAL_HOLD'" class="icon-btn" title="立即快速验证恢复" @click="channelAct(row,'quick-validate')"><ShieldCheck :size="16"/></button><button class="icon-btn" :title="row.lifecycleState==='MANUAL_HOLD'?'恢复':'暂停'" @click="channelAct(row,row.lifecycleState==='MANUAL_HOLD'?'resume-validation':'manual-hold')"><component :is="row.lifecycleState==='MANUAL_HOLD'?RefreshCw:Pause" :size="16"/></button></div></td></tr></tbody></table></div>
  ...
  <ModalShell v-if="modal==='policy'" title="新建分组策略" @close="close">
    <form class="modal-form" @submit.prevent="createPolicy">
      ...
      <label class="full"><span>测试模型</span><input v-model="form.probeModel" required maxlength="200" :disabled="!selectedPolicyTargetGroup" :placeholder="selectedPolicyTargetGroup?'输入模型名称':'请先选择目标分组'"/><small>已填入该分组的默认模型，可直接修改为任意模型名称</small></label>
      ...
      <label><span>倍率上限</span><input value="按目标分组自动获取（缓存 3 分钟）" disabled/></label><label><span>保底可用渠道数</span><input v-model="form.minAvailableChannels" type="number" min="1" max="100" required/></label>
  ...
  <ModalShell v-if="modal==='policy-edit'" title="编辑分组策略" @close="close">
    <form class="modal-form" @submit.prevent="updatePolicy">
      ...
      <label class="full"><span>测试模型</span><input v-model="form.probeModel" required maxlength="200" placeholder="输入模型名称"/><small>可以使用目标节点尚未枚举的新模型</small></label>
