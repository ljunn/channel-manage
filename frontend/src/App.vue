...
async function viewSource(id){
  ...
  try{
    ...
    Object.assign(form,{sourceGroupIDs:[],targetGroupIDs:[],targetID:writableTargets.value[0]?.id||'',priority:1000,concurrency:1})
...
async function deploySourceGroups(){
  ...
  await submit(()=>api(`/sources/${selectedSource.value}/deploy`,{method:'POST',body:body({targetID:form.targetID,sourceGroupIDs:form.sourceGroupIDs,targetGroupIDs:form.targetGroupIDs,priority:Number(form.priority||1000),concurrency:Number(form.concurrency||1)}),timeout}),`已自动创建 ${form.sourceGroupIDs.length} 个专用 Key 和 ${accountCount} 个独立托管账号，默认停止调度`)
}
async function openPolicy(){open('policy',{targetID:writableTargets.value[0]?.id||'',targetGroupID:'',mode:'PRICE',minSuccessRate:95,minSamples:5,confirmationFailures:3,cooldownMinutes:15});await loadPolicyTargetGroups()}
...
async function createPolicy(){await submit(()=>api('/policies',{method:'POST',body:body({name:form.name,scopeType:'TARGET_GROUP',scopeID:form.targetGroupID,config:{mode:form.mode,minSuccessRate:Number(form.minSuccessRate||95),minSamples:Number(form.minSamples||5),confirmationFailures:Number(form.confirmationFailures||3),cooldownMinutes:Number(form.cooldownMinutes||15)}})}),'分组策略草稿已创建')}
...
<template>
    ...
    <section class="workspace">
      ...
      <main>
        ...
        <template v-else-if="route==='/settings'">
          ...
          <div class="settings-layout"><section class="settings-section"><header><ShieldCheck :size="20"/><div><h2>安全闸门</h2></div></header><label class="toggle-row"><div><strong>影子模式</strong><small>开启时只评估，不写入目标节点，也不生成动作</small></div><input v-model="data.settings.shadow_mode" type="checkbox"/><span/></label><label class="toggle-row danger-row"><div><strong>紧急冻结</strong><small>保存后阻止全部远程写动作</small></div><input v-model="data.settings.emergency_freeze" type="checkbox"/><span/></label></section>
  ...
  <ModalShell v-if="modal==='policy'" title="新建分组策略" @close="close"><form class="modal-form" @submit.prevent="createPolicy"><label class="full"><span>策略名称</span><input v-model="form.name" required/></label><label><span>目标节点</span><select v-model="form.targetID" required @change="loadPolicyTargetGroups"><option v-for="item in writableTargets" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><label><span>目标分组</span><select v-model="form.targetGroupID" required><option value="" disabled>请选择</option><option v-for="group in policyTargetGroups" :key="group.id" :value="group.id">{{ group.name }} · {{ multiplierLabel(group.multiplier) }}</option></select></label><fieldset class="auth-mode full"><legend>排序模式</legend><label :class="{active:form.mode==='PRICE'}"><input v-model="form.mode" type="radio" value="PRICE"/><span>价格优先</span></label><label :class="{active:form.mode==='SPEED'}"><input v-model="form.mode" type="radio" value="SPEED"/><span>速度优先</span></label></fieldset><label><span>倍率上限</span><input value="按目标分组自动获取（缓存 3 分钟）" disabled/></label><label><span>自动优先级</span><input value="从 1000 开始依次排序" disabled/></label><label><span>最低成功率（%）</span><input v-model="form.minSuccessRate" type="number" min="0" max="100"/></label><label><span>最少样本</span><input v-model="form.minSamples" type="number" min="1"/></label><label><span>确认失败次数</span><input v-model="form.confirmationFailures" type="number" min="1"/></label><label><span>冷却时间（分钟）</span><input v-model="form.cooldownMinutes" type="number" min="1"/></label><footer class="full"><button type="button" class="btn" @click="close">取消</button><button class="btn primary">创建</button></footer></form></ModalShell>
