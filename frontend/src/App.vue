    ...
    <section class="workspace">
      ...
      <main>
        ...
        <template v-else-if="route==='/policies'">
          ...
          <div class="policy-list"><article v-for="row in filtered(data.policies)" :key="row.id" class="panel policy"><header><div><strong>{{ row.name }}</strong><small>{{ row.scopeType }} · v{{ row.activeVersion||1 }}</small></div><span :class="['badge',statusTone(row.status)]">{{ statusText(row.status) }}</span></header><dl><div><dt>倍率上限</dt><dd>目标分组自动值</dd></div><div><dt>最低成功率</dt><dd>{{ row.config.minSuccessRate||95 }}%</dd></div><div><dt>最少样本</dt><dd>{{ row.config.minSamples||5 }}</dd></div><div><dt>倍率缓存</dt><dd>{{ row.multiplierCacheSeconds||180 }} 秒</dd></div></dl><footer><button v-if="row.status!=='ACTIVE'" class="btn primary small" @click="activatePolicy(row)"><Play :size="14"/>启用</button><button class="btn small" @click="action(()=>api(`/policies/${row.id}/simulate`,{method:'POST'}),'模拟完成，结果已生成')"><BarChart3 :size="14"/>模拟</button></footer></article></div>
        </template>
