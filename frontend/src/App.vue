...
<template>
  ...
  <div v-else class="app-shell">
    <aside class="sidebar">
      <div class="brand"><span class="brand-mark"><Layers3 :size="19" /></span><div><strong>渠道管家</strong><small>{{ appVersion ? `v${appVersion}` : '版本读取中' }}</small></div></div>
      <nav>
    ...
    <button v-if="mobileOpen" class="mobile-backdrop" title="关闭导航" aria-label="关闭导航" @click="mobileOpen=false"/><aside v-if="mobileOpen" id="mobile-navigation" class="mobile-drawer" role="dialog" aria-modal="true" aria-label="主导航"><header class="brand"><span class="brand-mark"><Layers3 :size="19"/></span><strong>渠道管家</strong><button class="icon-btn" title="关闭" @click="mobileOpen=false"><X :size="18"/></button></header><nav><section v-for="group in nav" :key="group.label"><span class="nav-group-label">{{ group.label }}</span><a v-for="([path,label,Icon]) in group.items" :key="path" :href="`#${path}`" :class="{active:route===path}" :aria-current="route===path?'page':undefined"><component :is="Icon" :size="18"/>{{ label }}</a></section></nav></aside>
  </div>
