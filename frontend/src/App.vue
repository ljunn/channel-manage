...
function editPolicy(row){open('policy-edit',{id:row.id,name:row.name,targetName:row.targetName,targetGroupName:row.targetGroupName,mode:row.config.mode||'PRICE',minSuccessRate:row.config.minSuccessRate??95,minSamples:row.config.minSamples??5,confirmationFailures:row.config.confirmationFailures??3,cooldownMinutes:row.config.cooldownMinutes??15})}
function policyConfigPayload(){return {mode:form.mode,minSuccessRate:Number(form.minSuccessRate??95),minSamples:Number(form.minSamples??5),confirmationFailures:Number(form.confirmationFailures??3),cooldownMinutes:Number(form.cooldownMinutes??15)}}
