...
<template>
  ...
  <ModalShell v-if="modal==='policy'" title="新建分组策略" @close="close">
    <form class="modal-form" @submit.prevent="createPolicy">
      ...
      <label><span>优先级起点</span><input v-model="form.priorityStart" type="number" min="1" max="1000000" required/></label><label><span>优先级间隔</span><input v-model="form.priorityStep" type="number" min="1000" max="1000000" step="1000" required/><small>至少相差 1000，数字越大越靠后</small></label>
